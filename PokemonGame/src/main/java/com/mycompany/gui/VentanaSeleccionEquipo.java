...
/**
 * Pantalla previa a la batalla: muestra los Pokémon disponibles en la
 * Pokedex (los que ya vienen de la BD, vía Pokedex.getPokemones()) para
 * que el jugador arme su propio equipo, en vez de usar uno fijo por código.
 *
 ...
 */
public class VentanaSeleccionEquipo extends JFrame {
    ...
    public VentanaSeleccionEquipo(List<Pokemon> disponibles, int tamanoEquipo, Consumer<List<Pokemon>> onConfirmar) {
        super("Elige tu equipo");
        this.tamanoEquipo = tamanoEquipo;

        // ── Fondo general degradado (estilo "cielo Pokémon") ───────────────
        JPanel raiz = new JPanel(new BorderLayout(8, 10)) {
            @Override
            protected void paintComponent(Graphics g) {
        ...
        raiz.setBorder(BorderFactory.createEmptyBorder(0, 0, 12, 0));
        ...
        comenzar.addActionListener(e -> {
            dispose();
            onConfirmar.accept(seleccionados);
        });
