...
public class TablaTipos { //Utiliza el patron singleton para instanciar una unica vez la clase TablaTipos
    ...
    public double getMultiplicador(TipoPokemon atacante, TipoPokemon defensor) {
        ...
        if (info == null) {
            return 1.0; // neutral si no hay reglas
        }
        if (info.efectivos.contains(defensor)) {
            return 2.0;
        }
        if (info.inefectivos.contains(defensor)) {
            return 0.75;
        }
        return 1.0; // neutral
    }
